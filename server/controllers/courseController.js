  ...
  try {
    ...
    if (Array.isArray(req.body)) {
      ...
      for (const course of req.body) {
        const { title, code, department, unit, semester, program, level } =
          course;
        if (!title || !code || !unit || !semester || !program || !level) {
          return res.status(400).json({
            message:
              "All required fields (title, code, unit, semester, program, level) must be provided for each course",
          });
        }
        if (
          !mongoose.isValidObjectId(program) ||
          (department && !mongoose.isValidObjectId(department))
        ) {
          return res
            .status(400)
            .json({ message: "Invalid program or department ID" });
        }
        // Restrict to admin's department
        if (
          department &&
          !user.department.some((d) => d._id.toString() === department)
        ) {
          return res
            .status(403)
            .json({
              message: `You are not authorized to create courses for department ${department}`,
            });
        }
      }
    ...
    if (!title || !code || !unit || !semester || !program || !level) {
      return res.status(400).json({
        message:
          "All required fields (title, code, unit, semester, program, level) must be provided",
      });
    }
    if (
      !mongoose.isValidObjectId(program) ||
    ...
    if (error.code === 11000) {
      return res.status(409).json({
        message: `Course with code "${
          req.body.code || req.body[0]?.code
        }" already exists`,
      });
    }
  ...
  try {
    ...
    if (existingRegistration) {
      return res.status(409).json({
        success: false,
        message:
          "Already registered for this semester. Update your existing registration instead.",
      });
    }
    const uploads = {
    ...
    if (
      ...
      !Array.isArray(files.courseRegistrationSlip) ||
      files.courseRegistrationSlip.length === 0
    ) {
      return res.status(400).json({
        success: false,
    ...
    try {
      ...
      if (semester === "First Semester") {
        ...
        if (
          ...
          !Array.isArray(files.schoolFeesReceipt) ||
          files.schoolFeesReceipt.length === 0
        ) {
          return res.status(400).json({
            success: false,
        ...
        if (
          ...
          !Array.isArray(files.hallDuesReceipt) ||
          files.hallDuesReceipt.length === 0
        ) {
          return res.status(400).json({
            success: false,
        ...
        const departmentalDuesAmount = 3; // This should come from configuration
        ...
        const userWithDepartment = await User.findById(studentId).populate(
          "department",
          "name"
        );
        ...
        return res.status(200).json({
          success: true,
          message:
            "Documents uploaded successfully. Proceed to payment for departmental dues",
          data: {
        ...
        if (needsSchoolFeesPayment) {
          ...
          const paymentReference = generatePaymentReference();
          const schoolFeesAmount = 3.5; // This should come from configuration
          const payment = await Payment.create({
            student: studentId,
            registration: registration._id,
            semester,
...
export const getMyCourses = async (req, res) => {
  try {
    ...
    const payments = await Payment.find({ student: userId }).select(
      "semester amount status currency createdAt transactionId"
    );
