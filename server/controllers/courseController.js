...
export const registerCourses = async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(400).json({
        message: "Program, level, semester, and courseIds array are required",
      });
    }
    ...
    if (
      !mongoose.isValidObjectId(program) ||
      !courseIds.every((id) => mongoose.isValidObjectId(id))
    ) {
      return res
        .status(400)
        .json({ message: "Invalid program or course ID(s)" });
    }
    ...
    if (existingRegistration) {
      return res.status(409).json({
        message: "You have already registered for courses this semester",
      });
    }
    ...
    if (courses.length !== courseIds.length) {
      return res.status(400).json({
        message:
          "One or more course IDs are invalid or do not match the program, level, or semester",
      });
    }
    ...
    res
      .status(500)
      .json({ message: "Server error during course registration" });
  }
};
...
export const getMyCourses = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Server error while fetching your courses" });
  }
};
